...
export const WB_COMMODITY_IDS = ["wheat", "maize", "rice", "beef", "bananas", "cotton"];
const COMMODITY_MAP = {
...
function computeStats(audPrices) {
...
export async function fetchWorldBankData() {
    if (cache && Date.now() - cache.fetchedAt < CACHE_TTL_MS)
        return cache.data;
    const entries = await Promise.all(Object.entries(COMMODITY_MAP).map(async ([id, mapping]) => {
...
export function enrichCommodity(commodity, wbPoints) {
    const mapping = COMMODITY_MAP[commodity.id];
    if (!mapping || wbPoints.length === 0)
        return commodity;
    const audPrices = wbPoints.map((p) => mapping.toAud(p.value));
    const priceHistory = wbPoints.map((p) => ({
        date: `${p.date}-01`,
        price: mapping.toAud(p.value),
    }));
    return {
        ...commodity,
...
//# sourceMappingURL=worldBank.service.js.map