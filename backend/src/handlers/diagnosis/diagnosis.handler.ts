...
function formatDiagnosisAsText(diagnosis: DiagnosisResult): string {
  const parts: string[] = [
    ...
    `\n**Treatment:**`,
    diagnosis.treatment.map((t: string) => `• ${t}`).join("\n"),
  ];

  if (diagnosis.organicAlternatives?.length) {
    parts.push(`\n**Organic Alternatives:**`);
    parts.push(diagnosis.organicAlternatives.map((alt: string) => `• ${alt}`).join("\n"));
  }

  if (diagnosis.preventionTips?.length) {
    parts.push(`\n**Prevention Tips:**`);
    parts.push(diagnosis.preventionTips.map((tip: string) => `• ${tip}`).join("\n"));
  }
