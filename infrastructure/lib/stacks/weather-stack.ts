...
import { AgriSenseLambda } from '../constructs/lambda-function';
import { AgriSenseTable } from '../constructs/dynamo-table';
...
export class WeatherStack extends cdk.Stack {
  ...
  private createAlertsTable(stage: string): AgriSenseTable {
    return new AgriSenseTable(this, 'AlertsTable', {
      tableName: `AgriSense-${stage}-WeatherAlerts`,
  ...
  private createAlertsTopic(stage: string): sns.Topic {
    return new sns.Topic(this, 'AlertsTopic', {
      topicName: `AgriSense-${stage}-WeatherAlerts`,
    });
  }

  private buildCommonEnv(
    stage: string,
    alertsTable: AgriSenseTable,
  ...
  private grantPermissions(
    alertsTable: AgriSenseTable,
    alertsTopic: sns.Topic,
    lambdas: Record<string, AgriSenseLambda>
  ): void {
  ...
  private createPollingRule(stage: string, pollingLambda: AgriSenseLambda): void {
    const rule = new events.Rule(this, 'PollingRule', {
      ruleName: `AgriSense-${stage}-WeatherPolling`,
      schedule: events.Schedule.rate(cdk.Duration.minutes(30)),
    });
    rule.addTarget(new targets.LambdaFunction(pollingLambda.fn));
  }

  private createApiGateway(stage: string, lambdas: Record<string, AgriSenseLambda>): apigw.RestApi {
    const api = new apigw.RestApi(this, 'WeatherApi', {
      restApiName: `AgriSense-${stage}-WeatherApi`,
